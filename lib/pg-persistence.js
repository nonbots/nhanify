...
class Persistence {
  ...
  async findUser(username) {
    const FIND_USER = `SELECT * FROM users WHERE username ILIKE $1`;
    const result = await client.query(FIND_USER, [username]);
  ...
  async createUserTwitch(username) {
    const CREATE_USER = `INSERT INTO users (username) VALUES ($1) RETURNING id, username`;
    const result = await client.query(CREATE_USER, [username]);
    return result.rows[0];
  }
  ...
  async createPlaylist(title, isPrivate, creatorId) {
    ...
    SELECT $1, $2, $3 
    WHERE (SELECT COUNT(*) FROM playlists WHERE creator_id = $3) < 10 RETURNING id`;
    const result = await client.query(statement, [title, isPrivate, creatorId]);
    return result.rows[0];
  }
   ...
   */
  async addSong(title, videoId, playlistId, addedBy, durationSecs) {
    const statement = `INSERT INTO songs (title, video_id, playlist_id, creator_id, duration_sec) 
    SELECT $1, $2, $3, $4, $5
    WHERE (SELECT COUNT(*) FROM songs WHERE playlist_id = $3) < 50;`;
  ...
  async getPlaylistByUserPlaylistName(username, playlistName) {
    ...
    const statement = `SELECT playlists.id FROM users JOIN playlists ON users.id = playlists.creator_id WHERE users.username = $1 AND playlists.title = $2`;
