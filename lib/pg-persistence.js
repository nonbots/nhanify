...
class Persistence {
  ...
  async createPlaylist(title, isPrivate, creatorId) {
    ...
    SELECT $1, $2, $3 
    WHERE (SELECT COUNT(*) FROM playlists WHERE creator_id = $3) < 10;`
   ...
   */
  async addSong(title, videoId, playlistId, addedBy) {
    const statement = `INSERT INTO songs (title, video_id, playlist_id, creator_id) 
    SELECT $1, $2, $3, $4
    WHERE (SELECT COUNT(*) FROM songs WHERE playlist_id = $3) < 50;`;
    const result = await client.query(statement, [title, videoId, playlistId, addedBy]);
    return result;
  }
