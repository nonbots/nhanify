...
class Persistence {
  async addSong(title, url, videoId, playlistId, userId) {
    try {
      const statement = `INSERT INTO songs (title, url, video_id, playlist_id, user_id) VALUES ( $1, $2, $3, $4, $5);`;
      const result = await client.query(statement, [title, url, videoId, playlistId, userId]);
      return result.rowCount === 1;
    } catch (error) {
      if (error.constraint === "unique_video_id_playlist_id") return false;
      throw error;
    }
  }
  ...
  async getPlaylistInfoSongs(playlistId) {
    const statement = `
  ...
  async getPlaylist(playlistId) {
    const statement = `SELECT * FROM playlists WHERE id = $1`;
    const result = await client.query(statement, [playlistId]);
    return result.rows[0];
  }
  async editPlaylist(playlistId, newTitle, newVisibility) {
    console.log({ playlistId }, { newTitle }, { newVisibility });
