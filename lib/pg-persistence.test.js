...
describe("Persistence", () => {
  ...
  test("No playlist is return if userId is 2 and trying to access playlist 5", async () => {
    const expectedRowCount = true;
    expect(await persistence.getUserAuthorizedPlaylist(5, 2)).toBe(
      expectedRowCount,
    );
  });

  test("No playlist is return if userId is 1 and trying to access playlist 3", async () => {
    const expectedRowCount = false;
    expect(await persistence.getUserAuthorizedPlaylist(3, 1)).toBe(
      expectedRowCount,
    );
  });
  ...
  test("One playlist is return if userId is 1 and trying to access playlist 2", async () => {
    const expectedRowCount = true;
    expect(await persistence.getUserAuthorizedPlaylist(2, 1)).toBe(
      expectedRowCount,
    );
  });
